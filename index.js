(function () {
  ...
  var gv = (_gv = {
    heightSizeElement: 110,
    maxScrollElementSize: 51,
    minScrollSizeElement: 9,
    deltaScrollSizeElement: 42,
    countColumnSizeElement: 3,
    minScrollColorElement: 9,
    deltaScrollColorElement: 169,
    spaceScrollColorElement: 260
  }, _defineProperty(_gv, "minScrollColorElement", 12), _defineProperty(_gv, "widthBlockOfColorElement", 90), _defineProperty(_gv, "maxWidthPage", 1160), _defineProperty(_gv, "heightRowOfProductComposition", 61), _defineProperty(_gv, "marginBottomRowOfProdCompos", 57), _defineProperty(_gv, "nameOfButtonAdvice", 'СОСТАВ И УХОД'), _defineProperty(_gv, "newNameVideoButton", 'Посмотреть видео'), _defineProperty(_gv, "fixSliderMarginLeft", 45), _defineProperty(_gv, "missingProductMessage", 'Товар недоступен для заказа со склада'), _defineProperty(_gv, "seporatorLength", 7), _defineProperty(_gv, "maxPathLength", 65), _defineProperty(_gv, "widthSliderElement", 58), _defineProperty(_gv, "marginSliderElement", 44), _defineProperty(_gv, "fiveElementWidthSlider", 510), _defineProperty(_gv, "heightSortLink", 29), _defineProperty(_gv, "calcProcent", 0.0625), _defineProperty(_gv, "calcProcentIE", 20), _gv);
  var base64img = {
  ...
  Kameleoon.API.Core.runWhenElementPresent('.main-product-container', function (e) {
      ...
      */
      ...
      var createSizeElement = function createSizeElement(dropDown, sizeInfo) {
        ...
        */
        ...
        dropDown.querySelector('.dropdown-menu').style.height = "".concat(Math.ceil(arrayDropDownElem.length / 6) * 50, "px");
      ...
      var createPriceElement = function createPriceElement(price) {
        ...
        var createOutputPrice = function createOutputPrice() {
          ...
          if (promo == '-%') {
            ...
            outputPrice.innerHTML = "\n            <div class=\"kam_header-info_price\">".concat(price.textContent.split('руб.')[0], "<span class=\"kam_header-info_price-rub\">\u0440\u0443\u0431</span></div>\n            <div class=\"kam_header-info_promo\">").concat(getOldPrice(oldPrice), "<span class=\"kam_header-info_promo-rub\">\u0440\u0443\u0431</span><span class=\"kam_header-info_promo_procents\">").concat(promo, "</span></div>\n          ");
            ...
            var _Price = new mutationTracking(price, function () {
              ...
              oldPricePlace.innerHTML = "".concat(getOldPrice(oldPrice), "<span class=\"kam_header-info_promo-rub\">\u0440\u0443\u0431</span><span class=\"kam_header-info_promo_procents\">").concat(promo, "</span>");
            }, {
              childList: true,
              subtree: true,
              characterData: true
    ...
    */
    ...
    var createSecondElement = function createSecondElement(slides, liability) {
      var secondElement = document.createElement('div');
      secondElement.className = 'kam_second-element';
      /**
      *
      *
      */

      var createSliderElement = function createSliderElement() {
        var sliderElement = document.createElement('div');
        sliderElement.className = 'kam_slider-element';
        /**
        *
        *
        */

        var createShiftingElement = function createShiftingElement() {
          var shiftingElement = document.createElement('div');
          shiftingElement.className = 'kam_shifting-element';
          /**
          *
          *
          */

          var createGradationElements = function createGradationElements(slide) {
            var arrayRowsOfSlide = _toConsumableArray(slide.querySelectorAll('.row'));

            var maxHeightRow = 0;

            for (var i = 0, len = arrayRowsOfSlide.length; i < len; i++) {
              var heightRow = arrayRowsOfSlide[i].offsetHeight;
              maxHeightRow = heightRow > maxHeightRow ? heightRow : maxHeightRow;
            }

            for (var _i3 = 0, _len4 = arrayRowsOfSlide.length; _i3 < _len4; _i3++) {
              var graduationElement = document.createElement('div');
              graduationElement.className = 'kam_graduation';
              arrayRowsOfSlide[_i3].style.height = maxHeightRow + 'px';

              var graduation = arrayRowsOfSlide[_i3].querySelector('.graduation');

              if (!!graduation) {
                var widthGraduation = window.getComputedStyle(graduation, ':after').getPropertyValue('width');
                var intWidth = parseInt(widthGraduation);
                var value = void 0;

                if (intWidth % 10 === 0 && (intWidth !== graduation.offsetWidth || intWidth === 100) || intWidth === 0) {
                  value = intWidth / gv.calcProcentIE;
                } else {
                  value = intWidth * gv.calcProcent;
                }

                var fillGrad = 5 - value;

                for (var _i4 = 0; _i4 < value; _i4++) {
                  graduationElement.insertAdjacentHTML('beforeEnd', '<div class="kam_graduation-point kam_graduation-point-active"></div>');
                }

                for (var _i5 = 0; _i5 < fillGrad; _i5++) {
                  graduationElement.insertAdjacentHTML('beforeEnd', '<div class="kam_graduation-point"></div>');
                }

                graduation.innerHTML = '';

                arrayRowsOfSlide[_i3].querySelector('.title-description').insertAdjacentElement('beforeEnd', graduationElement);
              }
            }
          };

          createGradationElements(slides[0]);

          for (var i = 0, len = slides.length; i < len; i++) {
            if (slides[i].classList.contains('product-composition-component')) {
              (function () {
                var recomendations = _toConsumableArray(slides[i].querySelectorAll('.slidecontent > .row'));

                var fixLoadImg = function fixLoadImg(rows) {
                  for (var _i6 = 0, _len5 = rows.length; _i6 < _len5; _i6++) {
                    var img = rows[_i6].querySelector('img'),
                ...
                var createRecomendationsElement = function createRecomendationsElement() {
                  var recomendationsElement = document.createElement('div');
                  recomendationsElement.className = 'kam_recomendations';

                  for (var y = 0, _len = recomendations.length; y < _len; y++) {
                    recomendations[y].querySelector('.title-description').style.display = 'none';
                    recomendationsElement.insertAdjacentElement('beforeEnd', recomendations[y].querySelector('.libelle-description'));
                  }

                  return recomendationsElement;
                };

                var recomendationsElement = createRecomendationsElement(),
                    recomendationsElementHeight = recomendationsElement.offsetHeight,
                    washingElement = slides[i].querySelector('.slidecontent > .care-symbol') || false,
                    allWashingRow = washingElement ? washingElement.querySelectorAll('.block-description > .row') : false,
                    heightWashing = allWashingRow ? allWashingRow.length * gv.heightRowOfProductComposition + gv.marginBottomRowOfProdCompos : 0,
                    composeElement = slides[i].querySelector('.slidecontent > .table') || false,
                    heightCompose = composeElement ? composeElement.offsetHeight : 0;

                if (heightWashing > heightCompose) {
                  if (composeElement) {
                    composeElement.style.cssText = "\n                  height: ".concat(heightWashing, "px;\n                  margin-right: 36px;\n                ");
                  }
                } else {
                  if (washingElement) {
                    washingElement.style['margin-left'] = '36px';
                    washingElement.querySelector('.block-description').style.cssText = "\n                  height: ".concat(heightCompose - 61, "px;\n                ");
                  }
                }

                if (allWashingRow) {
                  fixLoadImg(_toConsumableArray(allWashingRow));
                  slides[i].querySelector('.care-symbol .block-description').insertAdjacentElement('afterEnd', recomendationsElement);
                } else {
                  if (composeElement) {
                    slides[i].querySelector('.block-description').insertAdjacentElement('afterEnd', recomendationsElement);
                  } else {
                    slides[i].insertAdjacentElement('afterEnd', recomendationsElement);
                  }

                  var styles = "\n                border-top: none;\n                padding: 10px !important;\n              ";
                  recomendationsElement.style.cssText = styles;
                }
              })();
            }

            var technicalContent = slides[i].querySelector('.technical-infos-box'),
                technicalContentAction = slides[i].querySelector('.technical-info-show-action');

            if (technicalContent && technicalContentAction) {
              technicalContent.classList.add('block');
              slides[i].querySelector('.technical-info-show-action').style.display = 'none';
            }

            shiftingElement.insertAdjacentElement('beforeEnd', slides[i]);
          }

          sliderElement.insertAdjacentElement('beforeEnd', shiftingElement);

          if (liability.textContent.trim() !== 'НАШИ ОБЯЗАТЕЛЬСТВА') {
            slides[0].insertAdjacentElement('beforeEnd', liability);
          } else {
        ...
        /**
        * create controller for slider
        *
        */

        (function () {
          var controllerElement = document.createElement('div');
          controllerElement.className = 'kam_controller-element';
          var shiftingElement = createShiftingElement();
          /**
          *
          *
          */
          ...
          var toSlideElement = function toSlideElement(num, button, missing) {
            ...
            var unActiveButtons = function unActiveButtons(buttons) {
              for (var i = 0, len = buttons.length; i < len; i++) {
                buttons[i].classList.remove('kam_controller-element_button-active');
              }
            };

            var activeButton = function activeButton(button) {
              button.classList.add('kam_controller-element_button-active');
            };

            var shiftSlide = function shiftSlide(slide, missing) {
              if (slide < 0) slide = 0;
              shiftingElement.style['margin-left'] = (-slide + missing) * gv.maxWidthPage + 'px';
              shiftingElement.style.height = shiftingElement.childNodes[slide].offsetHeight + 50 + 'px';
            };

            unActiveButtons(buttons);
            shiftSlide(num, missing);
            activeButton(button);
          };
          /**
          * create buttons for slider
          *
          */
          ...
          (function () {
            var _loop = function _loop(i, _y, len) {
              ...
              button.className = 'kam_controller-element_button';

              if (i == 0) {
                button.classList.add('kam_controller-element_button-active');
              }

              var unDisplaySlideTitle = function unDisplaySlideTitle(slide) {
                button.innerHTML = slide.textContent.trim();

                if (slide.textContent.trim() == 'СОСТАВ/ CОВЕТЫ') {
                  button.innerHTML = gv.nameOfButtonAdvice;
                }

                slide.style.display = 'none';
              };

              unDisplaySlideTitle(slides[i].querySelector('.box_anchor'));
              button.addEventListener('click', function () {
                return toSlideElement(i, button, _y);
              });
              controllerElements.push(button);
              controllerElement.insertAdjacentElement('beforeEnd', button);
              y = _y;
            };
          ...
          secondElement.insertAdjacentElement('afterBegin', controllerElement);
        })();

        var widthContoller = controllerElements.length === 2 ? '50%' : controllerElements.length === 1 ? '100%' : controllerElements.length === 0 ? false : true;

        if (controllerElements.length === 3) {
          [controllerElements[0], controllerElements[2]].map(function (button) {
            return button.style.cssText = 'width: calc(((100% - 380px * 3) / 2) + 380px); padding: 19px 95px 24px;';
          });
          controllerElements[0].style.textAlign = 'right';
          controllerElements[2].style.textAlign = 'left';
        } else {
          for (var i = 0; i < controllerElements.length; i++) {
            if (widthContoller !== true) {
              controllerElements[i].style.width = widthContoller;

              if (widthContoller === '50%') {
                controllerElements[i].style.padding = '19px 30px 24px';
              }
            }
          }
        }

        secondElement.insertAdjacentElement('beforeEnd', sliderElement);
        return widthContoller ? secondElement : document.createElement('div');
      };
    ...
    */
    ...
    Kameleoon.API.Core.runWhenElementPresent('.floor-complementary', function (e) {
      ...
      Kameleoon.API.Core.runWhenConditionTrue(function () {
        ...
        var fixSliderSecondBlock = function fixSliderSecondBlock() {
          return complementary.offsetHeight + 30 + 'px';
        };

        var secondElement = createSecondElement([complementary, technical, composition], liability);
        renderSecondElement(secondElement);
        complementary.parentNode.style.height = fixSliderSecondBlock();
      });
    });
  });
  Kameleoon.API.Core.runWhenElementPresent('.main-reviews-container', function (e) {
    ...
    var insertReviewElement = function insertReviewElement() {
      for (var _len6 = arguments.length, elements = new Array(_len6), _key = 0; _key < _len6; _key++) {
